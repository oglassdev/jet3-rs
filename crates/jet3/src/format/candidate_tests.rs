...
fn inspect(bytes: &[u8]) -> (Result<(), CandidateError>, ResourceBudget) {
    ...
    let result = RawJet3Candidate::inspect(source, &mut operation).map(|_| ());
    (result, operation)
}

#[test]
fn inspection_charges_only_the_signature_and_ignores_other_bytes() -> Result<(), CandidateError> {
    for (signature, expected) in [
        (b"Standard Jet DB", JetFileKind::Standard),
        (b"Jet System DB x", JetFileKind::System),
        (b"Temp Jet DB xyz", JetFileKind::Temporary),
    ] {
        let mut bytes = candidate_bytes(2, signature);
        ...
        assert_eq!(candidate.source().len(), ByteCount::new(4_096));
        ...
        assert_eq!(candidate.into_inner().len(), ByteCount::new(4_096));
    }
    Ok(())
}
...
fn every_truncation_before_complete_signature_is_structured() {
    ...
    for length in 0..19 {
        let (result, mut operation) = inspect(&complete[..length]);
        assert!(
            matches!(
                result,
...
#[test]
fn signature_is_checked_before_geometry() {
    let unknown = vec![0_u8; PAGE_BYTES + 1];
    let mut recognized = unknown.clone();
    recognized[4..19].copy_from_slice(b"Standard Jet DB");
    for (bytes, expected) in [
        (
            unknown,
            CandidateError::Signature(HeaderError::UnknownSignature { observed: [0; 15] }),
        ),
        (
            recognized,
        ...
        let (result, mut operation) = inspect(&bytes);
...
fn raw_page_read_delegates_exact_bytes_and_accounting() -> Result<(), Box<dyn StdError>> {
    let bytes = candidate_bytes(2, b"Standard Jet DB");
    ...
    assert_eq!(
        candidate.read_raw_page(PageNumber::new(2), &mut destination, &mut operation),
    ...
    assert_eq!(destination.as_slice(), &bytes[PAGE_BYTES..]);
    assert_eq!(
...
fn input_policy_is_rechecked_before_signature_or_geometry() {
    let source = CapturedGrowingSource {
        bytes: Rc::new(RefCell::new(candidate_bytes(1, b"Standard Jet DB"))),
...
fn inspection_and_page_reads_use_the_sources_captured_length() -> Result<(), Box<dyn StdError>> {
    let backing = Rc::new(RefCell::new(candidate_bytes(1, b"Standard Jet DB")));
    let source = CapturedGrowingSource {
