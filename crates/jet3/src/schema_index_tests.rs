...
impl Fixture {
    fn new(rows: &[&[RowValue<'_>]]) -> Result<Self, Box<dyn std::error::Error>> {
    ...
    fn table(&self) -> Result<TableDefinition, UpdateError> {
        let mut budget = budget();
        let mut database = DatabaseReader::open(self.path(), &mut budget)?;
        crate::update::indexed_writable_table(&mut database, b"Items", &mut budget)
    }
    fn create(
        &self,
        name: &[u8],
    ...
    fn drop_index(&self, index: &[u8]) -> Result<(), UpdateError> {
        edit_schema(
            self.path(),
...
fn table_rename_updates_the_catalog_and_preserves_user_storage() -> TestResult {
    ...
    let mut database = DatabaseReader::open(fixture.path(), &mut budget())?;
    let renamed = crate::update::indexed_writable_table(&mut database, b"Renamed", &mut budget())?;
...
fn create_table_keeps_existing_storage_and_accepts_later_rows() -> TestResult {
        ...
        )
    {
        ...
        let map = crate::mutation_map::MapBits::load(&mut database, locator, &mut budget())?;
    ...
    let mut database = DatabaseReader::open(fixture.path(), &mut budget())?;
    let added = crate::update::indexed_writable_table(&mut database, b"Added", &mut budget())?;
...
fn drop_table_removes_catalog_grants_and_releases_its_storage() -> TestResult {
    ...
    let mut b = budget();
    let mut database = DatabaseReader::open(fixture.path(), &mut b)?;
    assert!(crate::update::indexed_writable_table(&mut database, b"Items", &mut b).is_err());
    let global = crate::mutation_map::MapBits::load(
        &mut database,
        crate::mutation_map_write::global_locator(),
        &mut b,
