...
use crate::property_blob::{FIELD_BLOCK, PropertyBlob, TABLE_BLOCK};
use crate::{
...
fn text(
    ...
    match record {
        Some(record) => Ok(Some(crate::property_blob::owned(
            crate::column_property_reader::text(record)?,
            budget,
        )?)),
        None => Ok(None),
    }
...
impl<S: ReadAt> DatabaseReader<S> {
    ...
    pub fn table_properties(
        ...
        let blob = crate::column_property_values::Properties::load(
            self,
            catalog,
            &[table.root()],
            budget,
        ...
        let options = crate::column_property_reader::options(&blob, table.columns(), budget)?;
        let mut columns = Vec::new();
        crate::resource::reserve(&mut columns, table.columns().len(), budget)?;
