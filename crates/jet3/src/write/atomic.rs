...
impl PrivateCopy {
    ...
    pub(super) fn create_with_identity<F>(
        target: &Path,
        parent: &Path,
        capture_identity: F,
    ) -> io::Result<Self>
...
pub(super) fn prepare_private_for_removal(path: &Path) -> io::Result<()> {
...
#[cfg(not(windows))]
pub(super) fn prepare_private_for_removal(_path: &Path) -> io::Result<()> {
    Ok(())
}
...
#[cfg(windows)]
use super::atomic_windows::{path_identity, platform_publish};
