...
impl Fixture {
    ...
    pub(super) fn definition(&self) -> Result<crate::TableDefinition, Box<dyn StdError>> {
        let mut b = budget();
    ...
    pub(super) fn rows(&self) -> Result<Vec<(i32, RowLocator)>, Box<dyn StdError>> {
        let mut b = budget();
        let mut db = DatabaseReader::open(self.path(), &mut b)?;
    ...
    pub(super) fn validate(&self) -> TestResult {
        ...
        let mut db = DatabaseReader::open(self.path(), &mut b)?;
        let def = crate::write::update::indexed_writable_table(&mut db, b"Rows", &mut b)?;
        crate::index::mutation::load(&mut db, &def, &mut b)?;
...
pub(super) fn page(
    bytes: &[u8],
    number: crate::PageNumber,
) -> Result<&[u8; PAGE_BYTES], Box<dyn StdError>> {
...
fn preserve(
    ...
    let count = crate::index::tree::page::boundaries(page(after, leaf)?).count();
    assert_eq!(
        &after[base + 2..base + 4],
        &((1800 - count * 9) as u16).to_le_bytes()
    );
    assert_eq!(
        crate::index::tree::page::boundaries(page(after, leaf)?).collect::<Vec<_>>(),
    ...
    let dir = crate::row::directory::RowDirectory::validate(
        row.page(),
        table.root(),
        old,
        &mut budget(),
    )?;
    let next = crate::row::directory::RowDirectory::validate(
        row.page(),
        table.root(),
        new,
        &mut budget(),
