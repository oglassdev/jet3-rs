...
use crate::{
    ...
    WriteError, row::directory::RowDirectory,
};
...
pub(crate) fn plan(
    ...
    let storage = *graph.selected.last().ok_or(WriteError::NotFound("row"))?;
    ...
    let relative = {
        ...
        while let Some(row) = rows.next_row()? {
            ...
            if row.storage_locator() != storage {
                return Err(WriteError::Mismatch("overflow storage locator"));
            }
            let range = row
                .present_fixed_field_range(request.column)
                .ok_or(WriteError::Unsupported("null or variable field"))?;
            let bytes = row
                .field(request.column)
                .and_then(|field| field.raw_bytes())
                .filter(|bytes| bytes.len() == width && range.len() == width)
                .ok_or(WriteError::Mismatch("fixed field width"))?;
            before[..width].copy_from_slice(bytes);
            found = Some(range);
            break;
        }
        found.ok_or(WriteError::NotFound("row"))?
    };
    let row_range = entry.range();
    if relative.end > row_range.len() {
        return Err(WriteError::Mismatch("field outside row"));
    }
    let start = row_range
        .start
        .checked_add(relative.start)
        .ok_or(WriteError::Mismatch("field offset"))?;
    let end = start
        .checked_add(width)
        .ok_or(WriteError::Mismatch("field end"))?;
    if original_page.get(start..end) != Some(&before[..width]) {
        return Err(WriteError::Mismatch("source changed during planning"));
    }
...
pub(crate) fn writable_table(
    ...
    table: &[u8],
    budget: &mut ResourceBudget,
) -> Result<crate::TableDefinition, WriteError> {
...
pub(super) fn guarded_table(
    ...
    let mut root = None;
    {
        ...
        while let Some(record) = catalog.next_record()? {
            if record.class() == CatalogObjectClass::User && record.name().raw_bytes() == table {
                if root.is_some() {
                    return Err(WriteError::Mismatch("ambiguous table name"));
                }
    ...
    let definition =
        database.table_definition(root.ok_or(WriteError::NotFound("table"))?, budget)?;
    if definition.kind() != TableDefinitionKind::User {
        return Err(WriteError::Unsupported("non-user table"));
    }
    if !allow_indexes
...
pub(crate) fn require_writable_sort_order<S: crate::ReadAt>(
    ...
    match database.header().sort_order() {
        ...
        order => Err(WriteError::UnsupportedSortOrder {
