...
const SUPPORTED_FLAGS: u8 = UNIQUE_FLAG | REQUIRED_FLAG;
...
pub(crate) fn decode_indexes(
    ...
    for logical_index in 0..context.logical_count {
        ...
        let duplicate = names
            .iter()
            .any(|name: &DefinitionName| name.raw_bytes() == raw);
        if duplicate {
            return Err(IndexDefinitionError::InvalidLogicalName {
                logical_index,
                duplicate: true,
