...
fn first_row(
    ...
    let table = crate::write::update::indexed_writable_table(&mut db, name, &mut work)?;
...
fn stored_rules_refuse_row_writes_and_preserve_the_file() -> TestResult {
    ...
    let definition = crate::write::update::indexed_writable_table(&mut db, b"Items", &mut work)?;
...
fn unknown_sort_orders_are_readable_but_not_writable() -> TestResult {
    ...
    let root = {
        let mut db = DatabaseReader::open(&path, &mut work)?;
        crate::write::update::indexed_writable_table(&mut db, b"T", &mut work)?.root()
    };
...
fn chained_property_blobs_grow_and_shrink_without_touching_rows() -> TestResult {
    ...
    let [a, b, c, d] =
        [b'a', b'b', b'c', b'd'].map(|byte| [byte; crate::properties::column::MAX_TEXT_PROPERTY]);
    for edit in [
