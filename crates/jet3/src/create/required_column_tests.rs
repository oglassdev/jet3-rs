...
fn first_row(path: &std::path::Path) -> Result<crate::RowLocator, Box<dyn std::error::Error>> {
    ...
    let table = crate::write::update::indexed_writable_table(&mut db, b"Rows", &mut work)?;
...
fn required_columns_enforce_nulls_without_indexes_and_keep_scalar_exceptions() -> TestResult {
    ...
    let definition = crate::write::update::indexed_writable_table(&mut db, b"Rows", &mut work)?;
    let options = crate::properties::value_policy::options(&mut db, &definition, &mut work)?;
...
fn required_property_corruption_and_stored_nulls_are_reported() -> TestResult {
    ...
    let definition = crate::write::update::indexed_writable_table(&mut db, b"Rows", &mut budget())?;
    ...
    assert!(matches!(
        crate::properties::reader::decode(&encoded, definition.columns(), &mut budget()),
