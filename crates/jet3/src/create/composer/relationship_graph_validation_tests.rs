...
fn unsupported_rows_do_not_hide_malformed_known_reciprocals_or_duplicate_names() -> Result {
    ...
    let duplicate = super::relationship_graph_mutation_tests::fixture(&[first, first])?;
...
fn complete_inventory_rejects_an_endpoint_hidden_from_the_central_catalog() -> Result {
    ...
    let locator = db
        ...
        .ok_or("row absent")?
        .locator();
    let mut indexes = crate::index::mutation::load(&mut db, &central, &mut work)?;
...
fn consistent_child_rows_and_indexes_still_reject_an_orphan_key() -> Result {
    ...
    let mut indexes = crate::index::mutation::load(&mut db, &table, &mut work)?;
    indexes.replace(
        selected,
