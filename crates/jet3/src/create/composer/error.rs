...
#[error("Jet 3 bootstrap composition failed: {self:?}")]
pub enum ComposeError {
    ...
    Definition(#[from] TableDefinitionWriteError),
    /// A usage map could not be encoded.
    UsageMap(#[from] UsageMapWriteError),
    /// A page image could not be written.
    Page(#[from] PageImageError),
    /// A data row could not be encoded.
    Row(#[from] RowWriteError),
    /// The whole-file page plan could not be extended.
    WholeFile(#[from] WholeFilePlanError),
