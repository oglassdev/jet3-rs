...
pub(crate) fn initial_payload_start(
    ...
    let plan =
        crate::create::schema_plan::plan_table_schema(table, root.get(), first_create, budget)?;
...
pub(crate) fn encode_initial_row(
    ...
    for (ordinal, value) in values.iter().enumerate() {
        ...
        let options = columns.get(ordinal).map_or(
            crate::properties::reader::ColumnOptions {
                required: false,
                allow_zero_length: Some(false),
                validation_rule: false,
            },
            |column| crate::properties::reader::ColumnOptions {
