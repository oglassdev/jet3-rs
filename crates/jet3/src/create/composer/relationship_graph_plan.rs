...
use crate::{
    ColumnRef, IndexColumnSpec, IndexKind, IndexSpec, RelationshipSide, RelationshipSpec, TableRef,
    TableRows,
    create::{
        composer::*,
...
pub(super) fn push<T>(
    ...
    crate::format::resource::reserve(items, 1, budget)?;
    items.push(value);
    Ok(())
}
...
pub(super) fn resolve<'a>(
    ...
    for (position, relationship) in relationships.iter().enumerate() {
        ...
        if !(1..=crate::index::entry::MAX_FIELDS).contains(&relationship.fields.len()) {
        ...
        if child_id as usize >= crate::create::schema_plan::MAX_OBSERVED_INDEXES {
            return Err(ComposeError::Schema(
                crate::create::schema_plan::TableSchemaPlanError::UnobservedIndexCount {
                    count: child_id as usize + 1,
                    observed: crate::create::schema_plan::MAX_OBSERVED_INDEXES,
                },
            ));
        }
