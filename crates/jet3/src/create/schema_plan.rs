...
pub(super) const MAX_MAP_PAGE: u64 = 0x00ff_ffff;
...
pub(crate) fn plan_table_schema_for_order(
    ...
    crate::properties::column::check_for_order(spec.columns, spec.validation, order).map_err(
...
fn validate_name_bytes(
    ...
    match name.iter().enumerate().position(|(position, byte)| {
        !crate::catalog::name_key::supported_name_byte_for(*byte, order)
            || (position == 0 && *byte == b' ')
    }) {
...
fn assign_pages(
    ...
    let property_pages = match property_len {
        ...
        Some(len) if property_chained => {
            len.div_ceil(crate::long_value::writer::MAX_CHAINED_FRAGMENT)
        }
        Some(_) => 1,
    };
    let needed = 1
