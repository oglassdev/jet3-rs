...
impl RowPages {
    ...
    pub fn replace(
        ...
        let Some(after) = crate::row_update_page::replace_physical(
            row.page(),
            owner,
    ...
    pub fn remove(
        ...
        let index = self.position(database, row.page(), budget)?;
        let entry = &mut self.pages[index];
        entry.after = match crate::row_delete_page::remove_physical(
            row.page(),
            owner,
            entry.after.as_bytes(),
            row.slot(),
    ...
    pub fn stage(
        ...
        for entry in self.pages {
            let available =
                crate::allocation_patch::available(database, definition, entry.page, budget)?;
            ...
            let maps =
                crate::allocation_patch::plan(database, definition, entry.page, change, budget)?;
