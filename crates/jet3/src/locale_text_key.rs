...
use crate::{IndexDirection, SortOrder};

#[path = "locale_text_weights.rs"]
mod weights;
...
struct Weights {
    ...
    accents: [u16; 256],
    source_cost: [u8; 256],
    expansions: &'static [[u8; 2]],
}

fn weights(order: SortOrder) -> Option<&'static Weights> {
    Some(match order {
        SortOrder::Nordic => &weights::NORDIC,
        SortOrder::Spanish => &weights::SPANISH,
        SortOrder::Dutch => &weights::DUTCH,
        SortOrder::Cyrillic => &weights::CYRILLIC,
        SortOrder::Greek => &weights::GREEK,
        _ => return None,
    })
}
...
pub(crate) fn encode(
    ...
    if order == SortOrder::General {
        return crate::text_index_key::encode(value, maximum, direction, output);
    }
    ...
    if value
        .iter()
        .any(|&byte| crate::text::mapped_character(page, byte).is_none())
    {
        return None;
    }
...
pub(crate) fn prefix(
    ...
    if order == SortOrder::General {
        return crate::text_index_key::prefix(key, maximum, direction);
    }
    let weights = weights(order)?;
    let mask = u8::from(direction == IndexDirection::Descending).wrapping_neg();
