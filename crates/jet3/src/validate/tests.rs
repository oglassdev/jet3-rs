...
pub(super) fn validate(bytes: &[u8]) -> TestResult<Result<ValidationReport, ValidationError>> {
    let mut budget = budget();
    let mut database = open(bytes, &mut budget)?;
    Ok(database.validate(TextCodePage::Windows1252, &mut budget))
}

pub(super) fn definition(bytes: &[u8], name: &[u8]) -> TestResult<TableDefinition> {
    let mut budget = budget();
    let mut database = open(bytes, &mut budget)?;
...
pub(super) fn page_start(page: PageNumber) -> usize {
