...
fn only_catalog_lvprop_can_retain_empty_owned_payload_pages() -> TestResult {
    ...
    let mut database = open(&original, &mut work)?;
    let owned =
        crate::alloc::mutation_map::MapBits::load(&mut database, property_maps.owned(), &mut work)?;
    let page = *owned
    ...
    for (old, new) in [
        ...
        super::storage_tests::set_map_bit(&mut changed, old, page, false)?;
        super::storage_tests::set_map_bit(&mut changed, new, page, true)?;
    }
    assert!(
        matches!(validate(&changed)?, Err(ValidationError::Table { table, source:
        TableValidationError::Storage(StorageValidationError::Page {
