...
pub(crate) fn drop_table(
    ...
    let (catalog, row, grants, grant_rows, maps, definitions) = crate::schema::publish::apply(
        file,
        journal,
        budget,
        |database, budget| {
            ...
            let row = crate::schema::catalog::object(database, &catalog, table.root(), budget)?;
            let grants = crate::schema::catalog::table(database, b"MSysACEs", budget)?;
            let object = crate::schema::catalog::column(&grants, b"ObjectId")?;
            let mut grant_rows = Vec::new();
            let mut rows = database.rows(&grants, budget)?;
            while let Some(mut row) = rows.next_row()? {
                if matches!(crate::row::scalar_values::read_column(&mut row, object)?, RowValue::Long(value) if value as u32 as u64 == table.root().get())
                {
            ...
            let maps = crate::schema::storage::locators(&table, budget)?;
            let edits = crate::schema::storage::release_table(database, &table, budget)?;
            Ok((
                edits,
                (
                    catalog.root(),
