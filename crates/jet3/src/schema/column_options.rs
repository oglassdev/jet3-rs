        ...
        crate::schema::edit::apply(file, journal, budget, |database, budget| {
            ...
            for (property, change) in TextProperty::FIELD_ORDER.into_iter().zip(edit.text) {
                if let PropertyChange::Set(value) = change {
                    crate::properties::column::check_value_for(
                        value,
            ...
            let edited = if bytes.is_empty() && blob.blocks().is_empty() {
                ...
                crate::schema::properties::encode(&blob, budget)?
            };
            if !edited.is_empty() {
                crate::properties::reader::decode(&edited, table.columns(), budget)?;
            }
...
pub(crate) fn apply(
    ...
    for (name, value) in [(allow, edit.allow_zero_length), (required, edit.required)] {
        if let (Some(name), Some(value)) = (name, value) {
            block.set(
                crate::schema::properties::boolean(name, value, budget)?,
                budget,
            )?;
        }
    }
...
fn text_record(
    ...
    let record = if block == TABLE_BLOCK {
        ...
        let mut terminated = crate::properties::blob::owned(value, budget)?;
        crate::format::resource::reserve(&mut terminated, 1, budget)?;
        terminated.push(0);
        Record::new(
            property.flag(),
            property.field_kind(),
