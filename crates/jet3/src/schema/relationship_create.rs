...
pub(crate) fn create(
    ...
    if !spec.enforce {
        return crate::schema::relationship_catalog::create_unenforced(
            file,
            journal,
            &spec,
    ...
    let (
        ...
        let (object_id, folder) =
            crate::schema::relationship_catalog::object_identity(database, spec.name, budget)?;
        Ok((
    ...
    crate::schema::relationship_catalog::publish(
        file,
        journal,
        &spec,
        (parent_name, child_name),
...
fn add_endpoint(
    ...
    crate::schema::publish::apply(file, journal, budget, |database, budget| {
        ...
        let physical = if let Some(reused) = endpoint.reused {
            ...
            definition
                .indexes
                .push(crate::schema::definition::NamedRecord {
                    record: [0; 20],
                    name: endpoint.name,
                });
            reused
        } else {
            crate::schema::index::create(
                database,
                &table,
                IndexSpec {
                    name: endpoint.name,
        ...
        let mut record = [0; 20];
        crate::definition::column_writer::write_logical_record(
