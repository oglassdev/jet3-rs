    ...
    let root = {
        ...
        while let Some(record) = catalog.next_record()? {
            if record.name().raw_bytes() == name {
                if root.is_some() {
                    return Err(WriteError::Mismatch("ambiguous catalog table"));
                }
                root = record.table_definition();
            }
        }
        root.ok_or(WriteError::NotFound("catalog table"))?
    };
    Ok(database.table_definition(root, budget)?)
}

pub(crate) fn column(table: &TableDefinition, name: &[u8]) -> Result<ColumnOrdinal, WriteError> {
    table
        .columns()
        .iter()
        .find(|column| column.name().raw_bytes() == name)
        .map(|c| c.ordinal())
        .ok_or(WriteError::NotFound("catalog column"))
}
...
pub(crate) fn object(
    ...
    while let Some(mut row) = rows.next_row()? {
        ...
        {
            if found.is_some() {
                return Err(WriteError::Mismatch("duplicate catalog object"));
            }
            found = Some(row.locator());
        }
    }
    found.ok_or(WriteError::NotFound("catalog object"))
}
