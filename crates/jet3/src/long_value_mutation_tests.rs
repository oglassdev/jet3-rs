...
impl Fixture {
    ...
    fn definition(&self) -> Result<TableDefinition, Box<dyn std::error::Error>> {
        let mut b = budget();
    ...
    fn snapshot(&self) -> Result<BTreeMap<i32, Sample>, Box<dyn std::error::Error>> {
    ...
    fn validate(&self) -> TestResult {
        let mut b = budget();
        let mut db = DatabaseReader::open(self.path(), &mut b)?;
        let table = crate::update::indexed_writable_table(&mut db, b"Rows", &mut b)?;
        crate::long_value_mutation::LongValues::load(&mut db, &table, None, &mut b)?;
    ...
    for case in 0..5 {
        ...
        match case {
            ...
            2 => {
                ...
                let (after, _) = crate::row_insert_page::append(
                    target.page(),
                    super::OWNER,
                    &source,
                    b"unreferenced",
