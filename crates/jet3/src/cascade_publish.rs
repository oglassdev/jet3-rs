...
use crate::{ColumnOrdinal, FieldUpdate, PublishStage, RowDelete, RowUpdate};
        ...
        HE: StdError + Send + Sync + 'static,
    {
        ...
        crate::atomic::atomic_update_budgeted(
            ...
            |private, budget| -> Result<(), UpdateError> {
                ...
                crate::relationship_catalog::validate(&mut candidate, budget)?;
                Ok(())
            },
            hook,
        )?;
    ...
    fn stage(
            ...
            && let Change::Replace(_, original) = self.request
        {
            ...
            if count == 1 {
                ...
                crate::field_update::plan_fields(
                    &mut database,
                    &definition,
                    graph,
                    row.locator,
