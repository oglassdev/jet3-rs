...
pub(super) const SECOND_DATA: usize = 4;

pub(super) fn column_record(
    physical_type: u8,
    ordinal: u16,
...
pub(super) fn write_rows(page: &mut [u8], owner: u32, rows: &[(&[u8], u16)]) {
    page[0] = 1;
...
pub(super) fn database_bytes(pointer: [u8; 4], target_flags: u16, target_owner: u32) -> Vec<u8> {
    let mut bytes = crate::testkit::database_image(5);
