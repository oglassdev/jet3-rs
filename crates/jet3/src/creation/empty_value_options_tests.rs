...
fn malformed_properties_and_disabled_empty_values_preserve_input() -> Result<(), Box<dyn StdError>>
{
    ...
    for locator in [
        ...
        let bits = crate::mutation_map::MapBits::load(&mut db, locator, &mut work)?;
    ...
    assert!(matches!(
        crate::column_property_reader::decode(
...
fn text_only_properties_are_checked_before_publication() -> Result<(), Box<dyn StdError>> {
    ...
    let definition = crate::update::indexed_writable_table(&mut db, b"Other", &mut work)?;
    let map = crate::mutation_map::MapBits::load(&mut db, definition.maps().owned(), &mut work)?;
