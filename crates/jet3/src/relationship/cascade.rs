...
pub(crate) struct Plan<'a> {
    pub(super) rows: Vec<super::cascade_rows::Row>,
    pub(super) selected: usize,
    pub(super) request: Change<'a>,
...
pub(crate) fn prepare<'a>(
    ...
    let constraints = crate::relationship::catalog::component(database, target, table, budget)?;
    let mut rows = super::cascade_rows::load(database, &constraints, budget)?;
    ...
    for field in &mut row.fields {
        ...
        if let Some(value) = replacement {
            ...
            field.after = Some(super::cascade_rows::Value::copy(value, budget)?);
            field.explicit = true;
        }
    }
    ...
    for constraint in &constraints {
        ...
        if rows.iter().enumerate().any(|(position, row)| {
            ...
            let options =
                crate::properties::value_policy::options(database, &constraint.child, budget)?;
            crate::properties::value_policy::refuse_rules(&options, &constraint.child)?;
        }
    }
...
fn propagate(
    rows: &mut [super::cascade_rows::Row],
        ...
        for constraint in constraints {
            ...
            if !deleted {
                ...
                for value in &values[..constraint.parent_columns.len()] {
                    replacements.push(super::cascade_rows::Value::copy(*value, budget)?);
                }
            }
            for (child_position, child) in rows.iter_mut().enumerate() {
                ...
                if !super::cascade_rows::equal(&before, &key, budget)? {
                    continue;
                }
    ...
    for constraint in constraints {
        ...
        for row in rows {
            ...
            if let Some(key) = row.key(
                ...
                after,
                budget,
            )? && (!key::contains(&parents, &key, budget)?
                || (after
                    && constraint.self_reference_requires_existing_parent
                    && !key::contains(&previous, &key, budget)?))
            {
