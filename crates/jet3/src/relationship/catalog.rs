...
pub(super) struct Record {
    pub(super) order: crate::SortOrder,
    pub(super) name: Vec<u8>,
    pub(super) parent: Vec<u8>,
    pub(super) child: Vec<u8>,
    pub(super) parent_column: Vec<u8>,
    pub(super) child_column: Vec<u8>,
    pub(super) metadata: [i32; 3],
}
...
pub(super) fn interpreted(
    metadata: &[i32; 3],
) -> Option<crate::relationship::flags::RelationshipFlags> {
    let flags = crate::relationship::flags::RelationshipFlags::decode(metadata[0])?;
    let limit = if flags.enforced {
        crate::index::entry::MAX_FIELDS as i32
    } else {
        i32::MAX
    };
...
pub(super) fn enforced(records: &[&Record]) -> bool {
    records.first().is_some_and(|record| {
        crate::relationship::flags::RelationshipFlags::decode(record.metadata[0])
            .is_some_and(|flags| flags.enforced)
    })
}

pub(super) fn resolve<S: ReadAt>(
    database: &mut DatabaseReader<S>,
    records: &[&Record],
    budget: &mut ResourceBudget,
...
pub(super) fn incoming<S: ReadAt>(
    database: &mut DatabaseReader<S>,
    target: PageNumber,
    constraints: &[Constraint],
    budget: &mut ResourceBudget,
...
pub(super) fn read_records<S: ReadAt>(
    database: &mut DatabaseReader<S>,
    target: Option<&[u8]>,
    budget: &mut ResourceBudget,
) -> Result<Vec<Record>, WriteError> {
...
use super::groups::{groups, ordered};

pub(crate) use super::{component::component, validation::validate};
