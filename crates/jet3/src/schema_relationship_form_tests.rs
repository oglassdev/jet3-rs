...
fn unenforced_relationships_are_catalog_only_and_follow_table_edits() -> TestResult {
    ...
    let before = [fixture.table()?, {
        let mut b = budget();
        let mut database = DatabaseReader::open(fixture.path(), &mut b)?;
        crate::update::indexed_writable_table(&mut database, b"Children", &mut b)?
    }];
    ...
    let mut b = budget();
    let mut database = DatabaseReader::open(fixture.path(), &mut b)?;
    let items = crate::update::indexed_writable_table(&mut database, b"Items", &mut b)?;
    let children = crate::update::indexed_writable_table(&mut database, b"Children", &mut b)?;
    assert_eq!(items.indexes(), before[0].indexes());
    assert_eq!(children.relationships().count(), 1);
    let report = crate::relationship_catalog::validate(&mut database, &mut b)?;
