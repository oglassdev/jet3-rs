...
fn locale_key_bounds_and_undefined_bytes_are_checked() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for order in SortOrder::known() {
        ...
        for byte in 0..=255 {
            ...
            assert_eq!(
                length.is_some(),
                crate::format::text::mapped_character(page, byte).is_some()
            );
...
fn complete_native_locale_key_inventory() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for (name, order) in [
        ...
        for (raw, expected) in corpus[name]["keys"].as_object().ok_or("native keys")? {
            ...
            if raw
                .iter()
                .any(|&b| crate::format::text::mapped_character(page, b).is_none())
            {
            ...
            let length = crate::index::key::binary::shorten(&mut output[..length]);
            assert_eq!(
                output[..length],
