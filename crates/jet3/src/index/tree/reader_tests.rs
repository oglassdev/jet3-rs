...
pub(super) const ROW_PAGE: usize = 6;
...
pub(super) fn database_bytes(physical_type: u8, class: u8, size: u16) -> Vec<u8> {
...
pub(super) struct NodeSpec<'a> {
    ...
    pub(super) tail_child: usize,
    pub(super) prefix: &'a [u8],
    pub(super) entries: &'a [&'a [u8]],
}

pub(super) fn write_node(bytes: &mut [u8], spec: NodeSpec<'_>) {
...
pub(super) fn leaf_entry(key: &[u8], slot: u8) -> Vec<u8> {
    let mut entry = key.to_vec();
    entry.extend_from_slice(&[0, 0, ROW_PAGE as u8, slot]);
    entry
}

pub(super) fn branch_entry(key: &[u8], slot: u8, child: usize) -> Vec<u8> {
...
pub(super) fn traverse_with_limits(
    bytes: &[u8],
...
fn parse_page(
    ...
    kind: PageKind,
    page_number: usize,
) -> Result<crate::index::tree::page::ParsedNode, IndexTreeError> {
