...
use super::*;
use crate::{ColumnStorageClass, RowColumnLayout, RowValue, TableDefinition, TableDefinitionError};
...
fn resolves_physical_index_and_long_value_map_storage_ids_to_live_positions() -> TestResult {
    ...
    let physical = crate::physical_index_definition::decode_physical(
        0,
        [0; 8],
        raw,
        definition.columns(),
    ...
    raw[0] = 1;
    assert!(
        crate::physical_index_definition::decode_physical(
            0,
            [0; 8],
            raw,
            definition.columns(),
