"""Schema-complete synthetic M5R7 evidence for contract tests."""
...
def _operation(sample_id: str, phase: str, origin: dt.datetime) -> dict[str, Any]:
    ...
    return {"protocol_version": "1.0.0", "document_type": "dao_m5_operation_log", "experiment_id": "DAO-M5-COMPACT-CONFIRM-007", "sample_id": sample_id, "phase_id": phase, "worker_run_id": f"{sample_id}-{phase.upper()}", "started_at_utc": timestamp(origin, 1), "completed_at_utc": timestamp(origin, 7), "actions": actions, "status": "pass"}
...
def build_bundle(root: Path, present_companion: tuple[str, str] | None = None) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    for sample in plan["samples"]:
        ...
        for phase_index, phase in enumerate(PHASES, start=1):
            ...
            if phase != "compact":
                ...
                write_json(root / snapshot_locator, {"protocol_version": "1.0.0", "document_type": "dao_m5_snapshot", "experiment_id": "DAO-M5-COMPACT-CONFIRM-007", "sample_id": sid, "phase_id": phase, "captured_at_utc": timestamp(origin, 4), "captured_while_database_open": True, "dao_version": expected_version, "empty_user_schema": True, "user_table_count": 0})
    ...
    manifest = {"protocol_version": "1.0.0", "document_type": "dao_m5_bundle_manifest", "experiment_id": "DAO-M5-COMPACT-CONFIRM-007", "run_id": RUN_ID, "producer_commit": COMMIT, "created_at_utc": "2026-08-10T23:59:00Z", "sample_count": 108, "worker_count": 324, "m4_manifest_sha256": M4_MANIFEST_SHA256, "file_count": len(files), "bundle_tree_complete": True, "unexpected_files_present": False, "symlinks_or_reparses_present": False, "execution_status": "pass", "files": files}
