...
foreach($arm in $planValue.arms){
    ...
    foreach($values in $arm.rows){
        for($i=0;$i-lt $arm.fields.Count;$i++){
            ...
            if($null-ne $values[$i]){$variant=Variant $field $values[$i];if($field.type-eq 5){
                if($variant-isnot [Runtime.InteropServices.CurrencyWrapper]){throw 'Currency wrapper missing'}
                $memory=[Runtime.InteropServices.Marshal]::AllocCoTaskMem(16)
                try{
                    [Runtime.InteropServices.Marshal]::GetNativeVariantForObject($variant,$memory)
                    if([Runtime.InteropServices.Marshal]::ReadInt16($memory)-ne 6){throw 'Expected VT_CY'}
                    $scaled=[BitConverter]::ToInt64((From-Hex ([string]$values[$i])),0)
                    if([Runtime.InteropServices.Marshal]::ReadInt64($memory,8)-ne $scaled){throw 'Native Currency payload differs'}
                }finally{[Runtime.InteropServices.Marshal]::FreeCoTaskMem($memory)}
            }elseif($variant.GetType()-ne $rs.Fields.Item($field.name).Value.GetType()){throw 'Seek variant type differs'}}
            $count++
        }
...
Write-Output "Parser, $count typed conversions/Seek variants and all row JSON and native Currency VT_CY payloads passed; no DAO."
