...
import nullable_index_successor as experiment
import nullable_index_structure as structure


class NullableSuccessorTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def fixture(self, root):
        ...
        result = dict(document_type='dao_nullable_index_result', development_only=True,
                      ...
                      mutation_started=True, error=None, cleanup_failures=[], retention_failures=[], replicas=[])
    ...
    def test_failed_partial_job_retains_no_outcome_without_missing_file_crash(self):
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            plan, result = self.fixture(root)
            result['replicas'] = result['replicas'][:1]
            result['replicas'][0].pop('probes')
            result['error'] = {'endpoint': 'control/assign/0/1', 'message': 'initiating failure', 'stack': 'retained'}
            result['retention_failures'] = [{'file': 'unique-control-r1.mdb', 'message': 'copy failed'}]
            (root / 'unique-control-r1.mdb').unlink()
            (root / 'result.json').write_text(json.dumps(result))
            with patch.object(experiment, 'PLAN', plan):
                report = experiment.analyze(root)
            self.assertEqual(set(report['outcomes'].values()), {'no_outcome'})
            self.assertEqual(report['error'], result['error'])
            self.assertEqual(report['structures'], [])
            self.assertEqual(report['retention_failures'], result['retention_failures'])
