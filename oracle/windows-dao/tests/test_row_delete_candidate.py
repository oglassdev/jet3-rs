...
import row_delete_candidate as experiment
...
class CandidateTests(unittest.TestCase):
    ...
    def test_complete_classifier_and_failure_bindings(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            result=dict(document_type='dao_row_delete_candidate_result',producer_os='posix',source_revision=self.plan['source_revision'],plan_sha256=identity(experiment.PLAN)['sha256'],phase='complete',error=None,phases={},updates=[])
            for phase in ('create','observe'):
                document=dict(document_type='dao_row_delete_candidate_phase',phase=phase,plan_sha256=result['plan_sha256'],error=None,retention_failures=[],mutation_started=True,environment=dict(process_bits=32,provider='DAO.DBEngine.36'),observations=[],operations=[])
            ...
            with patch.object(experiment,'patch_check',return_value={'maps':{}}),patch.object(experiment,'shape',return_value=(None,None,{})):
                ...
                (root/'first-tail-r1-rust.mdb').write_bytes(b'changed');self.assertEqual(experiment.build_report(result,root,self.plan)['outcome'],'no_outcome')
    ...
    def test_exact_patch_preserves_payload_and_page_zero(self):
        ...
        rows=[dict(page=23,row=0,values=[1]),dict(page=23,row=1,values=[2])];maps=dict(owned=[23],available=[23]);arm=dict(selected_id=2)
