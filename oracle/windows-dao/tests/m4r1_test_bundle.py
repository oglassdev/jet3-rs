"""Synthetic, schema-complete M4R1 bundle builder for validator tests."""
...
from m4r1_bundle import build_analysis_from_stage
from m4r1_records import CHECKED_PLAN
...
def build_bundle(
    root: Path,
    present_companions: set[tuple[str, str]] | None = None,
) -> tuple[dict[str, Any], list[dict[str, Any]]]:
    present_companions = present_companions or set()
    ...
    for sample in plan["samples"]:
        ...
        for phase in ("creator", "reopen"):
            ...
            companion_locator = database_locator[:-4] + ".ldb"
            companion: dict[str, Any] = {
                "state": "absent",
                "path": companion_locator,
                "checked_after_worker_exit": True,
            }
            if (sample_id, phase) in present_companions:
                companion_payload = bytes([worker_ordinal % 256]) * 64
                companion_path = root / companion_locator
                companion_path.write_bytes(companion_payload)
                roles[companion_locator] = "companion"
                companion = {
                    "state": "present",
                    "path": companion_locator,
                    "bytes": 64,
                    ...
                    "file_identity": {
                        "volume_serial_number": "00000001",
                        "file_index": f"{10000 + worker_ordinal:016x}",
                        "link_count": 1,
                    },
            ...
            quiescence = {
                ...
                "phase_ordinal": phase_ordinal,
                "worker_run_id": worker_id,
                "worker_finished_at_utc": result["finished_at_utc"],
                "observation_started_at_utc": result["finished_at_utc"],
                "observation_completed_at_utc": result["finished_at_utc"],
                "worker_exit_wait_completed": True,
                "database": {
                    "path": database_locator,
                    "bytes": 2048,
                    "sha256": database_hash,
                    "prefix_sha256": digest(root / prefix_locator),
                    "file_identity": {
                        "volume_serial_number": "00000001",
                        "file_index": f"{sample['launch_ordinal'] * 2 - (1 if phase == 'creator' else 0):016x}",
                        "link_count": 1,
                    },
                    "exclusive_open_verified": True,
            ...
            write_json(root / quiescence_locator, quiescence)
            roles[quiescence_locator] = "post_worker_quiescence"
            worker = {
            ...
            phase_record = {
                ...
                "artifacts": {
                    ...
                    "post_worker_quiescence": artifact(root, quiescence_locator),
                },
        ...
        files.append(
            {
                ...
                "media_type": (
                    ...
                    if role in ("database", "prefix", "companion")
                    else "application/json"
                ),
            }
        )
    assert len(files) == 579 + len(present_companions)
    manifest = {
        "protocol_version": "1.0.0",
    ...
    os.utime(root, (fixed_time, fixed_time))
    return plan, records
