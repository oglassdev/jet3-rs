...
function Invoke-M4CheckedPhase {
    ...
    $quiescence = New-M4R1PostWorkerQuiescence `
        -Session $Session -Entries $Entries `
        -Invocation ([pscustomobject]$invocation) `
        -WorkerResult $result -Paths $paths `
        -DatabasePath (
            Get-M1PayloadPath -Session $Session -RelativePath $DatabasePath
        ) `
        -MaximumDatabaseBytes ([long]$Plan.bounds.max_database_bytes)
    $quiescencePath = Get-M1PayloadPath -Session $Session `
        -RelativePath ([string]$paths.quiescence)
    Invoke-M4ContractCommand -Context $Context `
        -ContractPath $ContractPath `
        -Arguments @(
            "validate-quiescence", "--bundle-root",
            $Session.StagingBundle, "--quiescence", $quiescencePath,
            "--result", $resultPath
        ) -Label "M4R1 $PhaseId post-worker quiescence validation"
    ...
    Add-Member -InputObject $result -MemberType NoteProperty `
        -Name controller_quiescence -Value $quiescence.binding
    Add-Member -InputObject $result -MemberType NoteProperty `
        -Name controller_quiescence_document -Value $quiescence.document
    Add-Member -InputObject $result -MemberType NoteProperty `
        -Name companion_artifact -Value $quiescence.companion_binding
    return $result
}
