...
$planPath=Join-Path $env:JET3_WORK 'eof-insert-candidate.plan.json'
$plan=Get-Content -LiteralPath $planPath -Raw|ConvertFrom-Json
foreach($entry in @(@('eof_insert_candidate.ps1',$PSCommandPath),@('field_update.ps1',(Join-Path $env:JET3_WORK 'field_update.ps1')))){
...
$result=@{document_type='dao_eof_insert_candidate_phase';phase=$phase;plan_sha256=(Identity $planPath).sha256;environment=@{process_bits=32;provider='DAO.DBEngine.36'};observations=@();operations=@();error=$null;retention_failures=@()}
try{
