...
$manifestPath = Join-Path $env:JET3_WORK 'index-capacity.json'
...
try {
    foreach ($pair in @(@($PSCommandPath, 'oracle/windows-dao/scripts/index_capacity.ps1'), @($scalarHelper, 'oracle/windows-dao/scripts/numeric_index_mutation.ps1'), @($helper, 'oracle/windows-dao/scripts/field_update.ps1'))) {
