...
def inventory(path):
    ...
    for name, expected_schema in SYSTEM_INDEXES.items():
        ...
        for index in definition['physical_indexes']:
            ...
            req(first == 0 and second == len({entry[:-4] for entry in entries}), 'system index prefixes/' + name)
            indexes.append({'ordinal': index['index'], 'flags': index['flags'], 'root': index['root'],
                            ...
                            'first_word': first, 'second_word': second,
                            'entries': [entry.hex() for entry in entries]})
        result[name] = {'schema': schema, 'row_count': len(rows), 'indexes': indexes}
    return result
