#!/usr/bin/env python3
"""EXP-0167: phased public tail deletion and DAO continuation, no retries."""
import argparse
import copy
...
PLAN = ROOT / 'oracle/windows-dao/acquisition/row-delete-candidate.plan.json'
SCRIPT = 'oracle/windows-dao/scripts/row_delete_candidate.ps1'
...
def patch_check(before, after, arm, receipt):
    ...
    if len(selected) != 1: raise ValueError('Delete Id not unique')
    row = selected[0]; page, slot = row['page'], row['row']
    if receipt != dict(root=table['root'],page=page,slot=slot): raise ValueError('Locator receipt')
    raw = layout.catalog._page(before,page,'target'); entries = layout.catalog._row_directory(raw,page)
    if len(entries)<2 or slot != len(entries)-1 or any(e['hidden'] or e['overflow'] or e['start']==e['end'] for e in entries):
        raise ValueError('Non-tail or flagged source')
    entry = entries[slot]; directory_end = 10+2*len(entries)
    if int.from_bytes(raw[2:4],'little') != entry['start']-directory_end or page not in maps['available']:
        raise ValueError('Source free/map metadata')
    expected_bytes = bytearray(before); base=page*2048; root=table['root']*2048
    expected_bytes[base+2:base+4]=(entry['end']-directory_end).to_bytes(2,'little')
    expected_bytes[base+10+2*slot:base+12+2*slot]=(0xc000|entry['end']).to_bytes(2,'little')
    expected_bytes[root+12:root+16]=(len(rows)-1).to_bytes(4,'little')
    if expected_bytes != after: raise ValueError('Exact deletion/payload/slack/page0 preservation')
    after_table, after_rows, after_maps = shape(after,arm)
    if maps != after_maps or sorted(r['values'] for r in after_rows) != sorted(r['values'] for r in rows if r is not row):
        raise ValueError('Deleted raw rows/maps')
    return dict(locator=receipt, row_count=after_table['row_count'], maps=maps,
        deleted_payload_hex=raw[entry['start']:entry['end']].hex(), changes=layout.changed_ranges(before,after),
        page0_unchanged=before[:2048]==after[:2048])


def entries(document, phase, plan):
    if (document['document_type']!='dao_row_delete_candidate_phase' or document['phase']!=phase
...
def build_report(result, outbox, plan):
    ...
    try:
        if (result['document_type']!='dao_row_delete_candidate_result' or result['producer_os']!='posix' or result['source_revision']!=plan['source_revision'] or result['plan_sha256']!=identity(PLAN)['sha256'] or result['phase']!='complete' or result['error'] is not None): raise ValueError('Coordinated phase/source failure')
        phases={}
    ...
    return dict(document_type='dao_row_delete_candidate_report',plan_sha256=identity(PLAN)['sha256'],outcome='observed_accepted' if not reasons else 'no_outcome',reasons=reasons,observations=observations,development_only=True,compatibility_claim=False,support_matrix_movement=False)
...
def dispatch(args):
    ...
    result=dict(document_type='dao_row_delete_candidate_result',producer_os=os.name,source_revision=plan['source_revision'],acquisition_revision=subprocess.check_output(['git','rev-parse','HEAD'],cwd=ROOT).decode().strip(),plan_sha256=identity(PLAN)['sha256'],phase='create',error=None,phases={},updates=[])
    try:
        for phase in ('create','observe'):
