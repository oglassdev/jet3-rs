#!/usr/bin/env python3
"""Pinned finite nullable Long index validation; one acquisition, no retries."""
...
PLAN = ROOT / 'oracle/windows-dao/acquisition/nullable-index-successor.plan.json'
SCRIPT = 'oracle/windows-dao/scripts/nullable_index_successor.ps1'
...
def rows_for(arm, name):
    ...
    count = 30000 if arm == 'composite' else 1200 if arm in ('composite-ignore', 'auto') else 12
    rows = []
    for n in range(count):
        if arm == 'required': a, b = n % 3, -n
        elif arm in ('composite', 'composite-ignore', 'auto'):
            a, b = [(None, None), (None, 1), (1, None), (n, -n)][n % 4]
        else: a, b = (None if n % 3 == 0 else n - 6), -n
        rows.append([a, n + 1 if arm == 'auto' else b, n])
    return rows


def order_key(row, table):
    return structure.key_bytes(row, table['indexes'][0]['fields'], [f['name'] for f in table['fields']])


def indexed_rows(rows, table):
    if not table['indexes'][0]['ignore_nulls']: return rows
    return [row for row in rows if any(value is not None for value in query_key(row, table))]
...
def expected_snapshot(arm, tables):
    ...
    for table in tables:
        ...
        value['rows'] = rows
        if table['indexes']:
            selected = indexed_rows(rows, table)
            by_key = {query_key(row, table): row for row in selected}
            value['traversal'] = sorted(selected, key=lambda row: order_key(row, table))
...
def normalize(snapshot, tables):
    ...
    for table in result.get('user_tables', []):
        ...
        rows = table['rows']
        valid &= all(len(row) == len(spec['fields']) and all(value is None or type(value) is int for value in row) for row in rows)
        row_set = {tuple(row) for row in rows}
        table['rows'] = sorted(rows, key=canonical)
        if spec['indexes']:
            traversal = table['traversal']
            selected = indexed_rows(rows, spec)
            ordered = sorted(traversal, key=canonical) == sorted(selected, key=canonical) and [order_key(row, spec) for row in traversal] == sorted(order_key(row, spec) for row in selected)
            valid &= ordered
            if ordered:
                table['traversal'] = sorted(traversal, key=canonical)
            existing = {query_key(row, spec) for row in selected}
            seeks = table['seek']
...
def analyze(outbox):
    ...
    if (result['document_type'] != 'dao_nullable_index_result' or result['plan_sha256'] != digest(PLAN)
    ...
    if (result['error'] is not None or result['cleanup_failures'] or result['retention_failures']
            or actual_inventory != expected_inventory):
        retained = {}
        for path in sorted(outbox.glob('*.mdb')):
            retained[path.name] = identity(path)
        report = dict(document_type='dao_nullable_index_report', development_only=True,
            compatibility_claim=False, support_movement=False, plan_sha256=digest(PLAN),
            result_sha256=digest(outbox / 'result.json'), outcomes={arm: 'no_outcome' for arm in ARMS},
            replicas=len(actual_inventory), error=result['error'], cleanup_failures=result['cleanup_failures'],
            retention_failures=result['retention_failures'], retained=retained, structures=[], rejection_probes=[])
    ...
    for item in result['replicas']:
        ...
        if arm in plan['rejection_probes']:
            for role in ('control', 'candidate'):
                probe = item['probes'][role]
                path = outbox / f'{arm}-{role}-probe-r{replica}.mdb'
                observation = probe['observation']
                if identity(path) != observation['after'] or probe['original'] != item[role]['after']:
                    raise ValueError('Probe retained/source identity mismatch')
                unchanged &= observation['before'] == observation['after']
                operation = probe['operation']
                reason = None
                try:
                    valid, snapshot = normalize(observation['snapshot'], tables)
                    passed = (valid and snapshot == expected and observation['status'] == 'pass'
                        and observation['endpoint'] == 'complete' and observation['error'] is None
                        and operation['status'] == 'rejected' and operation['endpoint'] == 'update'
                        and type(operation['hresult']) is int
                        and all(type(code) is int for code in operation['native_codes']))
                except (ValueError, KeyError, TypeError, IndexError) as error:
                    passed, reason = False, str(error)
                error_identity = (operation['native_codes'], operation['hresult'])
                probe_roles[role] = (passed, error_identity)
                probes.append(dict(arm=arm, replica=replica, role=role, passed=passed,
                    operation=operation, reason=reason, original=probe['original'], retained=identity(path)))
            controls_ok &= probe_roles['control'][0]
            probe_pass = probe_roles['candidate'][0] and probe_roles['candidate'][1] == probe_roles['control'][1]
            roles['candidate'] = (roles['candidate'][0] and probe_pass,) + roles['candidate'][1:] + (probe_roles['candidate'][1],)
    ...
    report = dict(document_type='dao_nullable_index_report', development_only=True, compatibility_claim=False,
                  support_movement=False, plan_sha256=digest(PLAN), result_sha256=digest(outbox / 'result.json'),
                  outcomes=outcomes, replicas=len(result['replicas']), candidates=plan['candidates'], structures=decoded, rejection_probes=probes)
...
def dispatch(args):
    ...
    shutil.copyfile(PLAN, inbox / 'nullable-index-successor.plan.json')
    for arm in ARMS:
