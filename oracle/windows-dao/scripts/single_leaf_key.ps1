...
function Set-Field($Rs,[string]$Name,$Value){$field=$Rs.Fields.Item($Name);try{$field.Value=$Value}finally{Release $field}}
...
$planPath=Join-Path $env:JET3_WORK 'single-leaf-key.plan.json';$plan=Get-Content -LiteralPath $planPath -Raw|ConvertFrom-Json
if((Identity $PSCommandPath).sha256-cne $plan.inputs.'oracle/windows-dao/scripts/single_leaf_key.ps1'-or (Identity $helper).sha256-cne $plan.inputs.'oracle/windows-dao/scripts/field_update.ps1'){throw 'Input pin mismatch'}
$script:mutationStarted=$false;$script:endpoint='prepare'
