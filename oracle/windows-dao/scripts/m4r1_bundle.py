...
from m4r1_analysis import build_analysis, canonical_analysis_bytes
from m4r1_campaign import validate_campaign_bindings_and_chronology
from m4r1_phase import (
    PhaseBindings,
...
ARTIFACT_ROLES = {
    ...
    "post_worker_quiescence": "post_worker_quiescence",
    "companion": "companion",
}
...
def load_document_v1_environment(
    ...
    if source is None:
        from m4r1_records import load_bounded_json
...
def _expected_manifest_bindings(
    ...
    for sample, record in zip(samples, records):
        ...
        for phase in ("creator", "reopen"):
            ...
            companion = phase_row["post_worker_quiescence"]["companion"]
            if companion["state"] == "present":
                add(companion["path"], "companion")
