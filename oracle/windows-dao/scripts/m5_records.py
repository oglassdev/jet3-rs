#!/usr/bin/env python3
"""Strict document loading and invocation bindings for DAO M5R5."""
...
SCHEMA_DIR = DAO_ROOT / "experiments" / "m5r4"
CHECKED_PLAN = DAO_ROOT / "experiments" / "m5" / "m5-compact-confirm-r5.plan.json"
...
def load_checked_plan(path: Path = CHECKED_PLAN) -> tuple[dict[str, Any], str]:
    ...
    if checked_hash != PLAN_SHA256:
        raise ValidationError("checked M5R5 plan hash differs from the compiled contract")
    ...
    if size != checked_size or digest != checked_hash or supplied != checked_bytes:
        raise ValidationError(f"{path}: bytes differ from the immutable checked M5R5 plan")
