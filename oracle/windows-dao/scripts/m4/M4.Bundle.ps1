...
$script:M4ExpectedPayloadFiles = 507
...
function ConvertTo-M4PhaseRecord {
    ...
    $phase = [ordered]@{
        ...
        post_close_file_observations = [ordered]@{
            ...
            lock_file_absent_after_close = [bool](
                $Result.post_close_file_observations.lock_file_absent_after_close
            )
        }
...
function Write-M4BundleManifest {
    ...
    if ($Entries.Count -ne $script:M4ExpectedPayloadFiles) {
        throw "M4 payload count differs from the exact 507-file contract."
    }
    ...
    $manifest = [ordered]@{
        ...
        sample_count = 36
        worker_count = 72
        file_count = $script:M4ExpectedPayloadFiles
