...
def _read_captured(
    ...
    *,
    ...
    document = None
    if role not in ("database", "prefix", "companion"):
...
class BundleSnapshot:
    ...
    def capture(cls, root: Path) -> BundleSnapshot:
        ...
        if companion_count < 0 or companion_count > 72:
            raise ValidationError("$.files: companion count exceeds 72")
        ...
        for entry in files:
            ...
            if role == "companion":
                ...
                if companion_bytes > 72 * 65536:
                    raise ValidationError(
                        "$.files: companion bytes exceed the campaign ceiling"
                    )
            require_equal(
