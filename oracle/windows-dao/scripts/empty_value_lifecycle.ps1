...
foreach ($source in @($helper, $scalarHelper)) {
    ...
    foreach ($name in $names) {
        ...
        if ($name -eq 'New-Control') {
            ...
            $definition = $definition.Replace('$fields.Append($field);', 'if ($name -eq "Items" -and [int]$spec[1] -in @(10,12)) { $field.AllowZeroLength = [bool]([string]$spec[0] -in $Case.allow_fields) }; $fields.Append($field);')
            $definition = $definition.Replace("@('Items', 'Notes')", '$Case.table_order')
            $definition = $definition.Replace('$Case.initial_rows', '$Case.creation_rows')
        }
...
$manifestPath = Join-Path $env:JET3_WORK 'empty-value-lifecycle.json'
...
try {
    foreach ($pair in @(@($PSCommandPath, 'oracle/windows-dao/scripts/empty_value_lifecycle.ps1'), @($scalarHelper, 'oracle/windows-dao/scripts/numeric_index_mutation.ps1'), @($helper, 'oracle/windows-dao/scripts/field_update.ps1'))) {
