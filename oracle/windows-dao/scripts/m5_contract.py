#!/usr/bin/env python3
"""Command-line entry point for checked DAO M5R4 evidence validation."""
...
def run(arguments: argparse.Namespace) -> str:
    ...
    if arguments.command == "validate-invocation":
        ...
        return "PASS: checked M5R4 invocation"
    if arguments.command == "validate-result":
        result, _, _ = validate_worker_result(arguments.bundle_root, arguments.result)
        return f"PASS: checked M5R4 worker result {result['sample_id']} {result['phase_id']}"
    if arguments.command == "validate-quiescence":
        ...
        return f"PASS: checked M5R4 quiescence {document['sample_id']} {document['database_role']}"
    if arguments.command == "validate-sample":
        document = validate_sample_record(arguments.bundle_root, arguments.record)
        return f"PASS: checked M5R4 sample {document['sample_id']}"
    if arguments.command == "build-analysis":
        ...
        return f"PASS: wrote checked M5R4 analysis to {arguments.output}"
    validated = validate_bundle(arguments.bundle_root, arguments.m4_bundle_root)
    return f"PASS: checked M5R4 bundle {validated['manifest']['run_id']}"
