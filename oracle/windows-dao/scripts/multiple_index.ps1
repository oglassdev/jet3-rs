...
function Variant($Definition,$Value){
    ...
    switch([int]$Definition.type){
        ...
        5{$scaled=[decimal][BitConverter]::ToInt64($raw,0)/[decimal]10000;return [Runtime.InteropServices.CurrencyWrapper]::new([decimal]$scaled)}
...
function New-Control([string]$Path,$Arm){
    ...
    try{
        ...
        foreach($spec in $Arm.indexes){
            $index=$table.CreateIndex([string]$spec.name);$index.Primary=[bool]$spec.primary;$index.Unique=[bool]$spec.unique;$index.Required=[bool]$spec.required;$index.IgnoreNulls=[bool]$spec.ignore
            for($i=0;$i-lt $spec.columns.Count;$i++){$key=$index.CreateField([string]$Arm.fields[[int]$spec.columns[$i]].name);if($spec.directions[$i]){$key.Attributes=1};$index.Fields.Append($key);Release $key;$key=$null}
            $table.Indexes.Append($index);Release $index;$index=$null
        }
        $db.TableDefs.Append($table);$rs=$db.OpenRecordset('Rows',2)
...
function Capture([string]$Path,$Arm){
    ...
    try{
        $captureName=[IO.Path]::GetFileName($Path);$script:endpoint="$captureName/open";$engine=New-Object -ComObject DAO.DBEngine.36;$db=$engine.OpenDatabase($Path,$false,$true)
        $script:endpoint="$captureName/metadata";$snapshot.version=[string]$db.Version;$snapshot.tables=@($db.TableDefs|ForEach-Object{[string]$_.Name}|Sort-Object)
        ...
        $script:endpoint="$captureName/rows";$rs=$db.OpenRecordset('Rows',4);$rows=New-Object Collections.ArrayList
        while(-not $rs.EOF){[void]$rows.Add((Row $rs $Arm));$rs.MoveNext()};$snapshot.rows=[object[]]$rows.ToArray();$rs.Close();Release $rs;$rs=$null
        $snapshot.traversals=@{};$snapshot.seeks=@{}
        foreach($spec in $Arm.indexes){
            $script:endpoint="$captureName/traversal/$($spec.name)";$rs=$db.OpenRecordset('Rows',1);$rs.Index=[string]$spec.name;$rs.MoveFirst();$rows=New-Object Collections.ArrayList
            while(-not $rs.EOF){[void]$rows.Add((Row $rs $Arm));$rs.MoveNext()};$snapshot.traversals[$spec.name]=[object[]]$rows.ToArray();$seeks=New-Object Collections.ArrayList
            $queryOrdinal=0
            foreach($query in $spec.queries){
                $script:endpoint="$captureName/seek/$($spec.name)/$queryOrdinal/variant/0";$first=Variant $Arm.fields[[int]$spec.columns[0]] $query[0]
                if($spec.columns.Count-eq 1){$script:endpoint="$captureName/seek/$($spec.name)/$queryOrdinal/call";$rs.Seek('=',$first)}else{
                    $script:endpoint="$captureName/seek/$($spec.name)/$queryOrdinal/variant/1";$second=Variant $Arm.fields[[int]$spec.columns[1]] $query[1]
                    $script:endpoint="$captureName/seek/$($spec.name)/$queryOrdinal/call";$rs.Seek('=',$first,$second)
                }
                $script:endpoint="$captureName/seek/$($spec.name)/$queryOrdinal/row";$row=if($rs.NoMatch){$null}else{Row $rs $Arm}
                [void]$seeks.Add(@{query=[object[]]$query;row=$row});$queryOrdinal++
            }
            $snapshot.seeks[$spec.name]=[object[]]$seeks.ToArray();$rs.Close();Release $rs;$rs=$null
        }
    }catch{$status='error';$errorDetail=Failure $_;$script:failure=$errorDetail}finally{if($null-ne $rs){try{$rs.Close()}catch{}};Release $rs;Release $table;if($null-ne $db){try{$db.Close()}catch{}};Release $db;Release $engine}
...
$planPath=Join-Path $env:JET3_WORK 'multiple-index.plan.json';$plan=Get-Content -LiteralPath $planPath -Raw|ConvertFrom-Json
if((Identity $PSCommandPath).sha256-cne $plan.inputs.'oracle/windows-dao/scripts/multiple_index.ps1'-or (Identity $helper).sha256-cne $plan.inputs.'oracle/windows-dao/scripts/fixed_field_successor.ps1'){throw 'Producer pin mismatch'}
$script:mutationStarted=$false;$script:endpoint='start';$script:failure=$null
$result=@{document_type='dao_multiple_index_result';plan_sha256=(Identity $planPath).sha256;source_revision=$plan.source_revision;environment=@{process_bits=32;provider='DAO.DBEngine.36';os=[Environment]::OSVersion.VersionString};mutation_started=$false;pairs=@();error=$null;retention_failures=@()}
try{
