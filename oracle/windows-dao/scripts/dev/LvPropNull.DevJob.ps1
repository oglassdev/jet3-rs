...
param(
    ...
    [string]$RunId,
    [Parameter(Mandatory = $true)]
    [string]$FixedCandidatePath,
    [Parameter(Mandatory = $true)]
    [string]$NullCandidatePath
)
...
$ExpectedCandidates = @{
    "candidate_fixed" = [pscustomobject]@{
        size = 47104
...
function Measure-Image {
    ...
    $endpoints = Test-AlphaImage -Path $Path
    return [ordered]@{
        role = $Role
...
foreach ($replica in 1..3) {
    ...
    try {
        ...
        foreach ($artifact in @(
            [pscustomobject]@{ path = $fixedPath; role = "candidate_fixed" },
            [pscustomobject]@{ path = $nullPath; role = "candidate_null" },
...
Write-JsonDocument -Path (Join-Path $RunRoot "lvprop-null-job-result.json") -Document $result
