...
function Assert-A3Bootstrap {
    ...
    if ([Environment]::OSVersion.Platform -ne [PlatformID]::Win32NT -or
        ...
        throw "A3 requires x86 Windows PowerShell 5 Desktop."
    }
    if ($Commit -cnotmatch "^[0-9a-f]{40}$") {
        throw "A3 producer commit must be lowercase SHA-1 hex."
    }
    if ($RunId -cnotmatch "^[A-Za-z0-9][A-Za-z0-9._-]{0,115}$" -or
        $MatrixJobId -cnotmatch "^[A-Za-z0-9][A-Za-z0-9._-]{0,127}$") {
        throw "A3 run or matrix job identifier is not protocol-valid."
    }
    ...
    )) { throw "A3 entrypoint differs from its repository binding." }
    $cursor = [IO.Path]::GetFullPath($Repository)
    while ($true) {
        $item = Get-Item -LiteralPath $cursor -Force
        if (-not $item.PSIsContainer -or
            ($item.Attributes -band [IO.FileAttributes]::ReparsePoint) -ne 0) {
            throw "A3 repository has a reparse component."
        }
    ...
    if ($headExit -ne 0 -or $head.Count -ne 1 -or
        ...
        $origin.Count -ne 1 -or [string]$origin[0] -cne $script:A3RepositoryUrl) {
        throw "A3 requires the exact clean repository and origin binding."
    }
    return $git
}

function Open-A3BootstrapSources {
    param(
        [Parameter(Mandatory = $true)][string]$Repository,
        [Parameter(Mandatory = $true)][string]$Commit,
    ...
    try {
        foreach ($relative in $RelativePaths) {
            ...
            if ($expectedExit -ne 0 -or $actualExit -ne 0 -or
                ...
                throw "A3 bootstrap source differs from the producer commit."
            }
            $stream = New-Object IO.FileStream(
...
function Read-A3JsonInput {
    ...
    if ($item.PSIsContainer -or $item.Length -lt 2 -or
        ...
        throw "A3 JSON input violates its byte or file bound."
    }
    $bytes = [IO.File]::ReadAllBytes($item.FullName)
    if ($bytes.Length -ne $item.Length -or
        ($bytes.Length -ge 3 -and $bytes[0] -eq 0xef -and
            $bytes[1] -eq 0xbb -and $bytes[2] -eq 0xbf)) {
        throw "A3 JSON input changed or contains a BOM."
    }
...
function Write-A3NewFile {
    ...
    if ($Bytes.Length -lt 1 -or $Bytes.Length -gt $MaximumBytes) {
        throw "A3 output violates its artifact byte ceiling."
    }
...
function Invoke-A3PythonValidation {
    param(
...
function Assert-A3PythonRuntime {
    ...
    $result = Invoke-BoundedChildProcess -Executable $Context.PythonPath `
        ...
        -CallerLabel "A3 Python runtime" -TimeoutSeconds 30 `
    ...
    if ($text -cnotmatch "^3\.13\.[0-9]+$" -or
        ...
        throw "A3 requires the preflight-bound Python 3.13 runtime."
    }
    return $text
}

function Assert-A3ExactPushedCommit {
    param([Parameter(Mandatory = $true)][pscustomobject]$Context)
    $remote = Invoke-BoundedChildProcess -Executable $Context.GitExecutable `
        -Arguments @(
            "-c", "credential.interactive=never", "-c", "core.askPass=",
            "ls-remote", "--heads", $script:A3RepositoryUrl
        ) -CallerLabel "A3 pushed commit binding" -TimeoutSeconds 30 `
    ...
    if ($advertised.Count -lt 1) {
        throw "A3 producer commit is not advertised by a pushed branch."
    }
}

function Invoke-A3ProviderProbe {
    param(
    ...
    if ([IO.File]::Exists($ProbePath)) {
        throw "A3 provider probe output already exists."
    }
    ...
    [void](Invoke-BoundedChildProcess -Executable $PowerShellPath `
        ...
        ) -CallerLabel "A3 DAO provider probe" -TimeoutSeconds 120 `
        -MaximumOutputBytes 1MB)
}

function New-A3EnvironmentBytes {
    param(
        [Parameter(Mandatory = $true)][pscustomobject]$Context,
...
function Assert-A3PlanIdentity {
    ...
    if ([string]$Plan.experiment_id -cne $script:A3ExperimentId -or
        ...
        [string]$Plan.implementation_rebinding.required_experiment_id -cne
            $script:A3ExperimentId -or
        [string]$Plan.implementation_rebinding.required_plan_path -cne
            $script:A3RequiredPlanPath) {
        throw "A3 rejects any plan whose experiment_id is not DAO-A3-ALLOCATION-MAPS-001."
    }
}

function Assert-A3RuntimeGate {
    param([Parameter(Mandatory = $true)][pscustomobject]$Plan)
    $expected = @(
        ...
        "a3_contract_validator_accepts_decisive_reports",
    ...
    if ([string]$Plan.execution_gate.status -cne "BLOCKED" -or
        ...
        throw "A3 preregistration runtime gate or worker bounds drifted."
    }
    for ($index = 0; $index -lt $expected.Count; $index++) {
        if ([string]$actual[$index] -cne [string]$expected[$index]) {
            throw "A3 preregistration blocking-requirement set drifted."
        }
    }
}

function Assert-A3ReplicaOutput {
    param(
        [Parameter(Mandatory = $true)][pscustomobject]$Context,
        [Parameter(Mandatory = $true)][string]$Root,
        [Parameter(Mandatory = $true)][string]$Validator,
    ...
    if ($directories.Count -ne $allowedDirectories.Count) {
        throw "A3 output directory inventory is not closed."
    }
    foreach ($directory in $directories) {
        if ($directory -cnotin $allowedDirectories) {
            throw "A3 output contains an unexpected directory."
        }
    }
    $manifestPath = Join-Path $rootPath $manifestRelative.Replace('/', '\')
    $manifestInput = Read-A3JsonInput -Path $manifestPath
    ...
    foreach ($record in $records) {
        ...
        if ($recordByPath.ContainsKey($relative)) {
            throw "A3 manifest contains a duplicate path."
        }
    ...
    if ($files.Count -ne ($records.Count + 1)) {
        throw "A3 output file inventory differs from its manifest."
    }
    foreach ($path in $files) {
        $item = Get-Item -LiteralPath $path -Force
        if ($item.PSIsContainer -or
            ($item.Attributes -band [IO.FileAttributes]::ReparsePoint) -ne 0) {
            throw "A3 output artifact is not a bounded ordinary file."
        }
        ...
        if (-not $recordByPath.ContainsKey($relative)) {
            throw "A3 output contains an unmanifested artifact."
        }
        ...
        if ([long]$record.size_bytes -ne [long]$item.Length -or
            ...
            throw "A3 artifact differs from its manifest hash or size."
        }
        if ([string]$record.role -ceq "page_blob") {
            if ($item.Length -ne 2048 -or
                [IO.Path]::GetFileNameWithoutExtension($path) -cne
                [string]$record.sha256) {
                throw "A3 page-store blob is not canonical content-addressed data."
            }
        }
    }
    ...
    $environment = (Read-A3JsonInput -Path $environmentPath).document
    ...
    foreach ($document in @($environment, $observation, $manifest)) {
        if ([string]$document.plan_sha256 -cne $script:A3PlanSha256 -or
            ...
            throw "A3 output document differs from its campaign binding."
        }
    }
    if ([string]$manifest.environment_sha256 -cne $EnvironmentSha256 -or
        ...
        throw "A3 output environment or provider binding drifted."
    }
    if ([string]$environment.matrix_job_id -cne $MatrixJobId -or
        ...
        throw "A3 output matrix-job binding drifted."
    }
    $checkpoints = @($observation.checkpoints)
    if ($checkpoints.Count -ne 25) { throw "A3 observation checkpoint count drifted." }
    $changedEntries = [long]0
    foreach ($checkpoint in $checkpoints) {
        $relative = [string]$checkpoint.page_index.path
        if (-not $recordByPath.ContainsKey($relative)) {
            throw "A3 checkpoint references an unmanifested page index."
        }
        $indexPath = Join-Path $rootPath $relative.Replace('/', '\')
        $indexInput = Read-A3JsonInput -Path $indexPath
        if ($indexInput.sha256 -cne [string]$checkpoint.page_index.sha256 -or
            ...
            throw "A3 checkpoint page-index reference is not byte-exact."
        }
        ...
        if ([string]$indexDocument.plan_sha256 -cne $script:A3PlanSha256 -or
            ...
            throw "A3 page index differs from its campaign binding."
        }
        $hashes = @($indexDocument.ordered_page_sha256)
        if ($hashes.Count -ne [long]$indexDocument.page_count) {
            throw "A3 ordered page hash count differs from page_count."
        }
        foreach ($digest in $hashes) {
            $blob = "page-store/$digest.page"
            if (-not $recordByPath.ContainsKey($blob)) {
                throw "A3 snapshot cannot be reconstructed from its page store."
            }
        }
        $changedEntries += @($indexDocument.changed_page_indices).Count
        Invoke-A3PythonValidation -Context $Context -Validator $Validator `
            -Path $indexPath -Label "A3 page-index schema validation"
    }
    if ($changedEntries -ne [long]$observation.changed_hash_entries -or
        $changedEntries -gt 65536) {
        throw "A3 changed-page accounting differs from the observation."
    }
    foreach ($validation in @(
        [pscustomobject]@{ path = $environmentPath; label = "A3 environment validation" },
        [pscustomobject]@{ path = $observationPath; label = "A3 observation validation" },
        [pscustomobject]@{ path = $manifestPath; label = "A3 replica manifest validation" }
    )) {
        Invoke-A3PythonValidation -Context $Context -Validator $Validator `
...
function Remove-A3PrivateWorkingRoot {
    param(
    ...
    if (-not $working.Equals($expected, [StringComparison]::OrdinalIgnoreCase)) {
        throw "Refusing A3 cleanup outside its private diagnostics boundary."
    }
...
function Move-A3FailedDatabase {
    param(
        [Parameter(Mandatory = $true)][string]$WorkingRoot,
        [Parameter(Mandatory = $true)][string]$Diagnostics,
    ...
    if (-not $working.Equals($expected, [StringComparison]::OrdinalIgnoreCase)) {
        throw "Refusing A3 failed-database retention outside its private boundary."
    }
    ...
    if ($item.PSIsContainer -or
        ...
        throw "A3 failed database is not a bounded ordinary file."
    }
    ...
    if ([IO.File]::Exists($destination) -or
        ...
        throw "A3 failed-database diagnostic already exists."
    }
    [IO.File]::Move($item.FullName, $destination)
}

function Invoke-A3ReplicaCampaign {
    param(
        [Parameter(Mandatory = $true)][string]$Repository,
    ...
    try {
        $script:A3Stage = "output-preflight"
        ...
        if ((Test-M1PathWithin -Path $Output -Parent $Repository) -or
            ...
            throw "A3 output and diagnostics must remain outside the repository and separate."
        }
        if (@([IO.Directory]::EnumerateFileSystemEntries($Output)).Count -ne 0) {
            throw "A3 OutputRoot must be empty."
        }
        $script:A3Stage = "plan-identity"
        $planPath = Join-Path $Repository $script:A3RequiredPlanPath
        $planInput = Read-A3JsonInput -Path $planPath -MaximumBytes 1MB
        if ($planInput.sha256 -cne $script:A3PlanSha256) {
            throw "A3 plan bytes differ from the frozen preregistration."
        }
        Assert-A3PlanIdentity -Plan $planInput.document
        $probePath = Join-Path $Diagnostics "provider-environment.json"
        $script:A3Stage = "provider-probe"
        Invoke-A3ProviderProbe -PowerShellPath $PowerShellPath `
        ...
        if ([string]$context.AcceptedProvider.prog_id -cne "DAO.DBEngine.36" -or
            ...
            throw "A3 requires one quote-free, hash-bound DAO.DBEngine.36 provider."
        }
        $pythonVersion = Assert-A3PythonRuntime -Context $context
        Assert-A3ExactPushedCommit -Context $context
        [void](Assert-M1RuntimeBinding -Context $context)
        $validator = Join-Path $Repository "oracle/windows-dao/scripts/a3_spec.py"
        Invoke-A3PythonValidation -Context $context -Validator $validator `
            -Path $planPath -Label "A3 immutable plan validation"
        Assert-A3RuntimeGate -Plan $planInput.document
        $campaignId = "a3-run-$RunId"
        if ($campaignId.Length -gt 128) { throw "A3 campaign id exceeds its bound." }
        ...
        $environmentBytes = New-A3EnvironmentBytes -Context $context `
        ...
        Write-A3NewFile -Path $environmentPath -Bytes $environmentBytes
        Invoke-A3PythonValidation -Context $context -Validator $validator `
            -Path $environmentPath -Label "A3 environment validation"
        [void][IO.Directory]::CreateDirectory($working)
        [void](New-A3ProgressFile -DiagnosticsRoot $Diagnostics -Replica $Replica)
        $workerPath = Join-Path $Repository `
            "oracle/windows-dao/scripts/a3/A3.Worker.ps1"
        $script:A3Stage = "replica-worker"
        [void](Invoke-BoundedChildProcess -Executable $PowerShellPath `
            -Arguments @(
                "-NoLogo", "-NoProfile", "-NonInteractive", "-ExecutionPolicy",
        ...
        $script:A3Stage = "self-verification"
        Assert-A3ReplicaOutput -Context $context -Root $Output `
        ...
        if ($null -ne $primary) {
            try {
                Move-A3FailedDatabase -WorkingRoot $working `
                    -Diagnostics $Diagnostics -Replica $Replica
            }
            catch {
                $removePrivate = $false
                [void]$secondaryErrors.Add(
                    "A3 failed-database retention also failed: " +
        ...
        if ($removePrivate -and [IO.Directory]::Exists($working)) {
            try {
                Remove-A3PrivateWorkingRoot -WorkingRoot $working `
                    -Diagnostics $Diagnostics
            }
            catch {
                [void]$secondaryErrors.Add(
                    "A3 private cleanup also failed: " +
                    [Convert]::ToString($_.Exception.Message)
                )
            }
        }
        if ($null -ne $context) {
            try { Close-M1PreflightContext -Context $context }
                ...
                [void]$secondaryErrors.Add(
                    "A3 preflight cleanup also failed: " +
                    [Convert]::ToString($_.Exception.Message)
                )
            }
        }
...
try {
    ...
    [void][IO.Directory]::CreateDirectory($diagnostics)
    $git = Assert-A3Bootstrap -Repository $repository -Commit $GitCommit
    $sources = @(
        "oracle/windows-dao/scripts/run-a3-replica.ps1",
        "oracle/windows-dao/scripts/a3/A3.Worker.ps1",
        ...
        "oracle/windows-dao/experiments/a3/replica-artifact-manifest.schema.json",
        "oracle/windows-dao/experiments/a3/environment.schema.json",
        "oracle/windows-dao/experiments/a3/analysis-report.schema.json",
        "oracle/windows-dao/experiments/a3/bundle-manifest.schema.json",
        "oracle/windows-dao/experiments/a3/dry-run-report.schema.json",
        "oracle/windows-dao/experiments/a3/holdout-structure-receipt.schema.json",
        "oracle/windows-dao/experiments/a3/derivation-candidates.schema.json",
        "oracle/windows-dao/experiments/a3/independent-validation-report.schema.json"
    )
    $streams = @(Open-A3BootstrapSources -Repository $repository `
    ...
    . (Join-Path $repository "oracle/windows-dao/scripts/a3/A3.Progress.ps1")
    $powerShellPath = [IO.Path]::GetFullPath(
        [Diagnostics.Process]::GetCurrentProcess().MainModule.FileName
    )
    Invoke-A3ReplicaCampaign -Repository $repository `
        -Output ([IO.Path]::GetFullPath($OutputRoot)) `
        -Diagnostics $diagnostics -PowerShellPath $powerShellPath `
        -ExecutedSources $sources
    Write-Output ("PASS: retained A3 replica {0:D2}" -f $Replica)
    exit 0
}
catch {
    $failure = $_
    try {
        Write-A3Failure -Root $DiagnosticsRoot -Stage $script:A3Stage `
            -Message $failure.Exception.Message
    }
    catch {
        [Console]::Error.WriteLine(
            "A3 failure diagnostic retention also failed: " + $_.Exception.Message
        )
    }
    [Console]::Error.WriteLine(
