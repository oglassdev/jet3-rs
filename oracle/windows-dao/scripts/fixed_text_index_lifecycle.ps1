...
$manifestPath = Join-Path $env:JET3_WORK 'fixed-text-index-lifecycle.json'
...
try {
    foreach ($pair in @(@($PSCommandPath, 'oracle/windows-dao/scripts/fixed_text_index_lifecycle.ps1'), @($scalarHelper, 'oracle/windows-dao/scripts/numeric_index_mutation.ps1'), @($helper, 'oracle/windows-dao/scripts/field_update.ps1'))) {
