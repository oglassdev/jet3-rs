...
function Set-Value($Recordset, $Definition, $Value) {
    ...
    try {
        ...
        switch ([int]$Definition.type) {
            ...
            8 { $field.Value = [datetime]::FromOADate([BitConverter]::ToDouble($raw, 0)) }
...
function Read-Row($Recordset, $Arm) {
    $values = New-Object object[] $Arm.fields.Count
...
$planPath = Join-Path $env:JET3_WORK 'scalar-index-layout.plan.json'
$plan = Get-Content -LiteralPath $planPath -Raw | ConvertFrom-Json
if ((Identity $PSCommandPath).sha256 -cne $plan.inputs.'oracle/windows-dao/scripts/scalar_index_layout.ps1') { throw 'Script pin mismatch' }
$mutationStarted = $false
$result = [ordered]@{
    document_type='dao_scalar_index_layout_result'; development_only=$true
