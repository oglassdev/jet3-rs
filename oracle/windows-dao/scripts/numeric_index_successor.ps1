...
function Capture([string]$Path,$Arm){
    ...
    try{
        ...
        $script:endpoint="$captureName/traversal";$rs=$db.OpenRecordset('Rows',1);$rs.Index='ByKey';$rs.MoveFirst();$rows=New-Object Collections.ArrayList
        ...
        foreach($query in $Arm.queries){
            $script:endpoint="$captureName/seek/$queryOrdinal/variant/0";$first=Variant $Arm.fields[0] $query[0]
            if($Arm.directions.Count-eq 1){$script:endpoint="$captureName/seek/$queryOrdinal/call";$rs.Seek('=',$first)}else{
                $script:endpoint="$captureName/seek/$queryOrdinal/variant/1";$second=Variant $Arm.fields[1] $query[1]
                $script:endpoint="$captureName/seek/$queryOrdinal/call";$rs.Seek('=',$first,$second)
            }
            $script:endpoint="$captureName/seek/$queryOrdinal/row";$row=if($rs.NoMatch){$null}else{Row $rs $Arm}
...
$planPath=Join-Path $env:JET3_WORK 'numeric-index-successor.plan.json';$plan=Get-Content -LiteralPath $planPath -Raw|ConvertFrom-Json
if((Identity $PSCommandPath).sha256-cne $plan.inputs.'oracle/windows-dao/scripts/numeric_index_successor.ps1'-or (Identity $helper).sha256-cne $plan.inputs.'oracle/windows-dao/scripts/fixed_field_successor.ps1'){throw 'Producer pin mismatch'}
