...
foreach ($source in @($helper, $scalarHelper)) {
    ...
    $names = if ($source -eq $helper) { @('Identity', 'Release', 'Write-Json') } else {
        @('Failure', 'Binary-Bytes', 'Variant', 'Set-Cell', 'Set-Row', 'Read-Row', 'Read-Names', 'Read-Fields', 'Read-Indexes', 'Read-Rows')
    }
...
function Mutate([string]$Path, $Case, $Operations) {
    $before = Identity $Path; $engine = $db = $rs = $null; $errorDetail = $null; $count = 0
    try {
        ...
        foreach ($operation in $Operations) {
            ...
            $script:endpoint = "$($Case.name)/$($operation.kind)/$id"
            if ($operation.kind -eq 'insert') {
                $rs.AddNew(); Set-Row $rs $Case $operation.row; $rs.Update()
            } else {
                $rs.Seek('=', [int]$operation.id)
                if ($rs.NoMatch) { throw 'Mutation Id absent' }
                if ($operation.kind -eq 'delete') { $rs.Delete() } else {
                    $rs.Edit()
                    switch ($operation.kind) {
                        'short' { Set-Cell $rs $Case 1 $operation.text; Set-Cell $rs $Case 2 $operation.binary }
                        'field' { Set-Cell $rs $Case ([int]$operation.column) $operation.value }
                        'replace' { Set-Row $rs $Case $operation.row }
                        default { throw 'Unknown mutation operation' }
                    }
                    $rs.Update()
                }
            }
        ...
        if ($null -ne $rs) { try { $rs.CancelUpdate() } catch {} }
    } finally {
        if ($null -ne $rs) { try { $rs.Close() } catch {} }; Release $rs
        if ($null -ne $db) { try { $db.Close() } catch {} }; Release $db; Release $engine
    }
    return @{ before = $before; after = (Identity $Path); count = $count; error = $errorDetail }
}
$script:endpoint = 'manifest'
$manifestPath = Join-Path $env:JET3_WORK 'row-overflow-lifecycle.json'
...
try {
    foreach ($pair in @(@($PSCommandPath, 'oracle/windows-dao/scripts/row_overflow_lifecycle.ps1'), @($scalarHelper, 'oracle/windows-dao/scripts/numeric_index_mutation.ps1'), @($helper, 'oracle/windows-dao/scripts/field_update.ps1'))) {
    ...
    foreach ($case in $manifest.cases) {
        ...
        try {
            if ($manifest.round -eq 'continuation') {
                ...
                $control = Join-Path $env:JET3_WORK "$($case.name)-control-working.mdb"; Copy-Item -LiteralPath (Join-Path $env:JET3_WORK $case.source_file) -Destination $control
                ...
                foreach ($stage in $case.stages) {
                    ...
                    if ($null -ne $checkpoint.mutation.error) { throw 'Checkpoint mutation failed' }
                    foreach ($role in @('candidate', 'control')) {
                ...
                foreach ($role in @('candidate', 'control')) {
                    ...
                    Copy-Item -LiteralPath (Join-Path $env:JET3_WORK "$($case.name)-$($case.stages[-1].name)-$role.mdb") -Destination $path
                    $native = @{ mutation = (Mutate $path $case $case.native); capture = (Capture $path $case) }; $outcome.native[$role] = $native
                    if ($null -ne $native.mutation.error -or $native.capture.status -ne 'pass') { throw 'Native follow-up capture failed' }
                }
            }
            $outcome.status = 'pass'
