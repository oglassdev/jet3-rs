"""Registry suites: jet3-cli requests build the candidate images, DAO replays the same recipe
on the VM, and the suite module compares both, independently decoding every image.

dao.py calls `run` for specs with a "registry" key: {"registry": module, ...}.
A suite module provides:

    SCRIPT                                  its PowerShell producer (dot-sources Common.ps1, Rows.ps1)
    EXTRA                                   optional extra guest files
    candidates(spec) -> images              the Rust candidates as a `recipes` image list
    prepare(images, revision, spec, results)
                                            checks the candidates and writes the manifest
    ...
    prepare_continue(images, outbox, output, revision)
                                            optional second round on native DAO outputs

Every stage keeps its inputs, requests, logs, VM outbox and comparison under the suite directory.
"""
...
class Run:
    def __init__(self, name: str, spec: dict, out: Path, context):
        ...
        self.report = {'suite': name, 'source': source, 'provenance': spec.get('provenance', []),
                       'registry': spec['registry'], 'stages': [], 'outcome': 'failed'}
    ...
    def run(self) -> dict:
        ...
        try:
            results = self.stage('generate', lambda: recipes.build(self.module.candidates(self.spec), images, self.out / 'requests'))
            self.stage('prepare', lambda: self.module.prepare(images, self.revision, self.spec, results) and None)
            ...
            if hasattr(self.module, 'prepare_continue'):
                ...
                self.stage('continuation-prepare', lambda: self.module.prepare_continue(
                    images, self.out / 'dao', continued, self.revision) and None)
