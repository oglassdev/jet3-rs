# M4R1 companion-aware version/encryption file-prefix campaign

M4R1 is the companion-aware revision of the bounded, descriptive DAO-only M4
experiment. Its experiment ID is `DAO-M4-HEADER-DISCRIMINATOR-002`, and its
preregistration is `EXP-0014`. The original `EXP-0011` plan and the complete
`experiments/m4/` protocol remain unchanged historical records.
Execution binds the distinct evidence ref
`refs/heads/codex/m4r1-companion-aware`; it may not reuse the original M4 ref.

The revision was required after execution of the original protocol encountered
a persistent 64-byte `.ldb` sibling for the first `dbVersion20` sample after
DAO close and worker cleanup. That operational observation did not authorize
inspection or analysis of the MDB prefix, and no original-M4 bundle was
published. M4R1 changes only post-worker quiescence and companion retention. It
does not change the scientific design.

The declarative plan is `m4-header-discriminator-r1.plan.json`. The copied,
...
M4R1 retains exactly the original six conditions, six replicas per condition,
...
a worker success condition in M4R1.
...
remain separate, and a schema-valid document alone is never evidence. M4R1
