# M5 compact-copy confirmation campaign, revision 4

M5 revision 4 is a bounded, descriptive, DAO-only experiment. It asks the
...
The additive declarative plan is `m5-compact-confirm-r4.plan.json`; its stable
experiment ID is `DAO-M5-COMPACT-CONFIRM-004`. Every earlier plan and README
remains immutable. `EXP-0021` records the failed pre-COM M5R3 attempt and
`EXP-0022` records this revision. `SRC-0019` records the numeric `dbDecrypt`
...
`EXP-0018` and the failed M5R3 attempt in `EXP-0021`. That attempt stopped in
controller preflight, before any worker launch or COM activation, because the
plan's 180-second worker timeout exceeded the already-enforced 120-second
shared bounded-process hard ceiling. R4 changes only that timeout to 120
seconds and the operational identities required for an additive revision.
...
semantically identical to M5R3. No provider observation or MDB byte informed
this amendment. Any later change after M5R4 execution requires another
...
host is bound to the exact clean pushed R4 producer commit and revised remote
ref. `EXP-0020` already records the checked controller, isolated workers,
analysis, complete-bundle validator, and focused contract gates. R4 must bind
those components to the additive plan and schemas without weakening them.
...
records, and the analysis report remain unchanged. The worker timeout is
reduced from 180 seconds to 120 seconds so it equals, and cannot exceed, the
pre-existing shared bounded-process hard ceiling. The shared ceiling itself is
not weakened or increased.
...
M5 revision 4 proves no ability to read, create, update, validate, encrypt,
