# M5 compact-copy confirmation campaign, revision 7

M5 revision 7 is a bounded, descriptive, DAO-only experiment. It asks the
...
experiment ID is `DAO-M5-COMPACT-CONFIRM-007`. Every earlier plan and README
remains immutable. `EXP-0030` records the failed M5R6 worker attempt and
`EXP-0031` records this revision. `SRC-0019` records the numeric `dbDecrypt`
...
`EXP-0018` and the failed M5R6 attempt in `EXP-0030`. That attempt completed
the first source and compact DAO phases, but compact-result validation rejected
an empty prefix path produced when Windows PowerShell coerced an intentionally
absent prefix locator from null to an empty typed string. R7 changes only the
internal optional-locator binding so null remains null and any non-null value
must be a nonempty string, plus the operational identities required for an
additive revision.
...
semantically identical to M5R6. No provider result or MDB byte informed this
amendment. Any later change after M5R7 execution requires another
...
host is bound to the exact clean pushed R7 producer commit and revised remote
ref. `EXP-0029` already records the checked controller, isolated workers,
analysis, complete-bundle validator, and focused contract gates. R7 must bind
those components to the additive plan and schemas without weakening them. The
artifact builder must emit JSON null for the absent compact-input prefix and a
checked nonempty artifact reference for every retained prefix.
...
M5 revision 7 proves no ability to read, create, update, validate, encrypt,
