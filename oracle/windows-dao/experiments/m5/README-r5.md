# M5 compact-copy confirmation campaign, revision 5

M5 revision 5 is a bounded, descriptive, DAO-only experiment. It asks the
...
experiment ID is `DAO-M5-COMPACT-CONFIRM-005`. Every earlier plan and README
remains immutable. `EXP-0024` records the failed pre-COM M5R4 attempt and
`EXP-0025` records this revision. `SRC-0019` records the numeric `dbDecrypt`
...
`EXP-0018` and the failed M5R4 attempt in `EXP-0024`. That attempt stopped in
isolated-worker preflight, before COM activation, because the worker called
`Assert-M1NoReparseComponents` before loading the already exact-source-bound
`M1.PublicationPaths.ps1` helper that defines it. R5 changes only the worker
preflight load order and the operational identities required for an additive
revision.
...
semantically identical to M5R4. No provider observation or MDB byte informed
this amendment. Any later change after M5R5 execution requires another
...
host is bound to the exact clean pushed R5 producer commit and revised remote
ref. `EXP-0023` already records the checked controller, isolated workers,
analysis, complete-bundle validator, and focused contract gates. R5 must bind
those components to the additive plan and schemas without weakening them. The
isolated worker must load the exact-source-bound publication-path helper before
calling any function that uses `Assert-M1NoReparseComponents`.
...
M5 revision 5 proves no ability to read, create, update, validate, encrypt,
