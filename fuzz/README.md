...
one borrowed payload and performs no payload-sized allocation. The checked
...
signatures, and exact/partial Jet 3 geometry. `corpus/manifest.json` records
each seed's stable ID, purpose, exact bytes and hash, origin, environment,
rights, and reproduction command.
...
identify a Jet version or validate a database. The header target does not
replace the required database-opening, catalog, table-definition, row, index,
or long-value parsers. Checked malformed-corpus execution, ten-minute
acceptance runs, resource monitoring, and the other requirements of validation
gate G5 remain release blockers.
