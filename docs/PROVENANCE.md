...
### EXP-0064 — Derived cross-producer long-value comparison projection

- Recorded: 2026-08-28, OpenAI Codex
- Kind: additive derived protocol rule from already recorded local exploratory
  observations; no new acquisition or compatibility result
- Question: Which Memo/OLE bytes can protocol 1.2 producers compare without
  making Jet storage form or external page/slot location a semantic fact?
- Origin: only the inline, single-page, chained, payload-length, Memo source
  byte, OLE byte, and 12-byte external-reference observations already recorded
  in `EXP-0061`; no new MDB, provider run, or third-party implementation was
  inspected
- Environment: repository-only derivation and shared-vector validation on
  Linux; no DAO or COM execution
- Protocol: define Memo/OLE typed-value `raw_hex` as the exact logical payload
  bytes for every storage form. Exclude the inline header, external header,
  link bytes, locators, page framing, and row-directory bytes. Retain an exact
  external 12-byte Rust header only under comparison-excluded
  `producer_extensions`, keyed by the final canonical table/row/value JSON
  pointer plus `jet_external_long_value_header`. Require the retained binary
  `value` and `raw_hex` to be the same 24 lowercase hexadecimal digits.
- Artifacts:
  `oracle/windows-dao/protocol/v1_2/fixtures/long-value-comparison-vectors.tsv`,
  SHA-256
  `941713af349c89f3dbcfb2e2ed1ca02b48011a1bd1c271fbfba5c248ff132c26`,
  consumed by the Rust and Python protocol tests
- Observation: none; the vector uses repository-authored synthetic payloads,
  headers, and independently recorded SHA-256 expectations to define a
  deterministic projection from the previously recorded storage facts
- Interpretation: equal logical Memo/OLE payloads must serialize and hash
  equally across inline, single-page, chained, and distinct external locators.
  Physical headers remain lossless Rust diagnostics but are never a required
  DAO fact. This rule does not establish equal producer output, DAO
  compatibility, or any support-matrix verification transition.
- Usage: `file:crates/jet3-testkit/src/semantic_snapshot.rs`;
  `file:crates/jet3-testkit/src/semantic_snapshot_long.rs`;
  `file:crates/jet3-testkit/src/semantic_snapshot_convert.rs`;
  `file:crates/jet3-testkit/src/semantic_protocol.rs`;
  `file:crates/jet3-testkit/src/semantic_protocol_validation.rs`;
  `file:oracle/windows-dao/scripts/validate_protocol_v1_2.py`;
...
### EXP-0065 — Derived cross-producer rejected-opening outcome normalization

- Recorded: 2026-08-28, OpenAI Codex
- Kind: additive derived protocol rule from already recorded reader error
  classes and preregistered scenario semantics; no new acquisition or
  compatibility result
- Question: How can protocol 1.2 represent the three planned unsupported-open
  outcomes without treating a rejected input as an ordinary empty database or
  claiming semantic allocation traversal?
- Origin: the structured `DatabaseFormatError` variants implemented from
  `EXP-0056`, the protocol 1.2 `expected_error` scenarios already generated by
  `build_v1_2_inventory.py`, and the existing closed opening-branch registry;
  no MDB implementation, provider output, DAO execution, or new database byte
  was inspected
- Environment: repository-only derivation, synthetic header tests, and shared
  schema/vector validation on Linux; no DAO or COM execution
- Protocol: normalize exact Rust `UnsupportedVersion`,
  `EncryptedOrUnsupported`, and `PasswordedOrUnsupported` format rejections to
  `unsupported_version`, `encrypted_database`, and `password_protected`.
  Represent them as `opening_failure` outcomes without semantic database
  members. Bind the scenario id, immutable staged-input SHA-256, build-bound
  producer revision, and normalized class in both artifacts. Require the
  receipt's exact opening branches and set `allocated_set_sha256` to `null`
  because allocation traversal is not applicable. No other opening error is
  eligible for normalization.
- Artifacts:
  `oracle/windows-dao/protocol/v1_2/fixtures/rejected-format-normalization-vectors.tsv`,
  SHA-256
  `5a6cd5f1da9653a142ab8a08dc6a53802089ad858ee5dca35a49600698017219`,
  consumed by the Rust and Python protocol tests
- Observation: none; the shared vector names only repository-authored type and
  protocol identifiers and contains no MDB or provider bytes
- Interpretation: a Rust-produced expected-error bundle can be validated and
  compared under protocol 1.2 without inventing an empty database or a page-set
  digest. This rule establishes deterministic artifact semantics only; it is
  not DAO evidence, a compatibility claim, or a verification-level transition.
- Usage: `file:crates/jet3-cli/src/main.rs`;
  `file:crates/jet3-testkit/src/semantic_outcome.rs`;
  `file:crates/jet3-testkit/src/semantic_json.rs`;
  `file:crates/jet3-testkit/src/semantic_protocol.rs`;
  `file:crates/jet3-testkit/src/semantic_snapshot.rs`;
  `file:oracle/windows-dao/scripts/validate_protocol_v1_2.py`;
  `file:oracle/windows-dao/protocol/v1_2/README.md`;
  `file:oracle/windows-dao/protocol/v1_2/canonical-semantic-snapshot.schema.json`;
  `file:oracle/windows-dao/protocol/v1_2/coverage-receipt.schema.json`
- Rights: repository-authored derived rule and fixture; no provider bytes are
  committed or redistributed
- Review: pending independent review
