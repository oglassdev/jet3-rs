...
including fixed-buffer primitive encoding, generic Jet signature recognition,
Jet 3 candidate inspection, Jet 3 page-geometry arithmetic, bounded
complete-page reads, fixed-size commit-region reads, and bounded sequential
raw-page streaming without adding benchmark dependencies to the production
workspace.
...
- `BinaryWriter` little-endian `u32` encoding and exact cumulative accounting
  across a complete rewrite after seeking;
...
The writer harness is isolated in `binary_writer_benchmark.rs`. Its
project-authored deterministic `u32` generator uses only a zero-based word
index and fixed constants. Both the first-pass and complemented rewrite words
are precomputed before timing. Before registering each measured size, an
untimed preflight executes both cases and checks the complete output bytes,
final position, remaining capacity, writer and budget encoded-byte counters,
and aggregate total-work counter. The expected output hashes are retained in
`manifest.json` and independently regenerated by the benchmark manifest tests.

Criterion allocates each 64-byte through 1-MiB output buffer during batched
setup. Timed code creates a `BinaryWriter`, writes one complete precomputed
primitive pass, and, for the rewrite cases, seeks to byte zero and writes a
precomputed complemented second pass. Every iteration uses exact encoded-byte
and aggregate-work ceilings. The timed closure returns its owned output,
budget, and captured writer state so Criterion destroys them after the
measurement interval. The harness emits no MDB structure and makes no
database-performance or compatibility claim.
